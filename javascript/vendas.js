...
});// javascript/vendas.js - VERSÃO FINAL COM BOTÃO DE INTERESSE NO CARD
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const sendNotification = async (product, type) => {
        if (!product || product.vendedorId === currentUser.uid) return;
        ...
        try {
            ...
            console.log(`Notificação de tipo '${type}' enviada!`);
        } catch (error) {
            console.error("Erro ao enviar notificação:", error);
        }
    };
    ...
    const renderNotifications = (notifications) => {
        ...
        notifications.forEach(notif => {
            ...
            if (notif.tipo === 'like') {
                message = `<strong>${notif.remetenteNome}</strong> curtiu seu serviço <strong>"${notif.produtoNome}"</strong>.`;
            } else if (notif.tipo === 'interest') {
                message = `🚀 <strong>${notif.remetenteNome}</strong> teve <strong>INTERESSE</strong> no serviço <strong>"${notif.produtoNome}"</strong>! Entre em contato.`;
            }
    ...
    const handleInterestClick = async (productId) => {
        if (!currentUser) {
            alert("Você precisa estar logado para demonstrar interesse.");
            return;
        }
        
        alert("Interesse registrado! O vendedor será notificado.");
        
        try {
            const product = allProducts.find(p => p.id === productId);
            if (product) {
                await sendNotification(product, 'interest');
            }
        } catch (error) {
    ...
    const showProductModal = async (productId) => {
        ...
        const interestButtonHtml = `
            <button class="btn-interest" data-product-id="${product.id}">
        ...
        modalBody.innerHTML = `
            ...
            <div class="modal-actions-container">
                ${interestButtonHtml}
                <div class="product-meta" style="border:none; padding:0;">${likeButtonHtml}</div>
            </div>
    ...
    const renderProducts = (productsToRender) => {
        ...
        productsToRender.forEach((product) => {
            ...
            // NOVO BOTÃO DE INTERESSE NO CARD
            const interestButtonHtml = `
                <button class="btn-interest card-btn-interest" data-product-id="${product.id}">
                    Tenho Interesse
                </button>`;
            ...
            const productMeta = document.createElement('div');
            productMeta.className = 'product-meta';
            // Agora adicionamos os dois botões
    ...
    productGrid.addEventListener('click', (event) => {
        const likeBtn = event.target.closest('.like-btn');
        const interestBtn = event.target.closest('.btn-interest'); // Detecta clique no novo botão
        const cardWrapper = event.target.closest('.card-content-wrapper');
        
        if (likeBtn) { handleLikeClick(likeBtn); } 
        else if (interestBtn) { handleInterestClick(interestBtn.dataset.productId); }
        else if (cardWrapper) { showProductModal(cardWrapper.dataset.id); }
    });

    modalBody.addEventListener('click', (event) => {
        const likeBtn = event.target.closest('.like-btn-modal');
        const interestBtn = event.target.closest('.btn-interest');
        
        if (likeBtn) { handleLikeClick(likeBtn); }
        if (interestBtn) { handleInterestClick(interestBtn.dataset.productId); }
    });
